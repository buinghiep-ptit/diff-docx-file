  ...
  const generateDiff = useCallback(() => {
    try {
      ...
      const processTableWithSections = (originalTable, editedTable) => {
        ...
        // Track processed rows for deletion detection
        const processedOriginalRows = new Set();
        ...
        for (let i = 0; i < minSectionCount; i++) {
          ...
          for (let j = 0; j < minRowCount; j++) {
            ...
            processedOriginalRows.add(origRow);
          }
          ...
          // Handle deleted rows in this section
          if (origSection.rows.length > editSection.rows.length) {
            // Find the tbody where we should insert deleted rows
            let targetTbody = editedTable.querySelector("tbody");
            if (!targetTbody) {
              // If no tbody exists, create one
              targetTbody = document.createElement("tbody");
              editedTable.appendChild(targetTbody);
            }

            // Insert deleted rows from this section
            for (let j = minRowCount; j < origSection.rows.length; j++) {
              const origRow = origSection.rows[j];
              const deletedRow = markRowAsDeleted(origRow);

              // Insert after the last processed row in this section
              if (
                j === minRowCount &&
                minRowCount > 0 &&
                editSection.rows.length > 0
              ) {
                // Insert after the last row in this edited section
                const lastEditedSectionRow =
                  editSection.rows[editSection.rows.length - 1];
                if (lastEditedSectionRow.nextSibling) {
                  targetTbody.insertBefore(
                    deletedRow,
                    lastEditedSectionRow.nextSibling
                  );
                } else {
                  targetTbody.appendChild(deletedRow);
                }
              } else {
                // Just append to end of tbody
                targetTbody.appendChild(deletedRow);
              }
            }
          }
        ...
        if (originalSections.length > editedSections.length) {
          ...
          // Insert deleted sections
          for (let i = minSectionCount; i < originalSections.length; i++) {
            const origSection = originalSections[i];

            // Mark each row in the deleted section
            origSection.rows.forEach((row) => {
              if (!processedOriginalRows.has(row)) {
                const deletedRow = markRowAsDeleted(row);
                targetTbody.appendChild(deletedRow);
              }
            });
          }
      ...
      const processComplexTable = (originalTable, editedTable) => {
        ...
        matchedOriginalNonIndexedRows.forEach((row) => {
          processedOriginalRows.add(row);
        });
        ...
        // Handle deleted rows - find rows in original that weren't processed
        // Find the tbody where we should insert deleted rows
        let targetTbody = editedTable.querySelector("tbody");
        if (!targetTbody) {
          // If no tbody exists, create one
        ...
        // Insert deleted rows (original rows that weren't processed)
        const unprocessedOriginalRows = Array.from(originalRows).filter(
          (row) => !processedOriginalRows.has(row)
        );

        if (unprocessedOriginalRows.length > 0) {
          // For each unprocessed original row, create a deleted row marker
          unprocessedOriginalRows.forEach((origRow) => {
            const deletedRow = markRowAsDeleted(origRow);
            if (deletedRow) {
              // Find appropriate insertion point - try to maintain relative position
              const rowIndex = Array.from(originalRows).indexOf(origRow);

              // Insert the deleted row at an appropriate position
              if (rowIndex >= 0 && rowIndex < editedRows.length) {
                // Try to insert at corresponding position if possible
                targetTbody.insertBefore(
                  deletedRow,
                  editedRows[rowIndex] || null
                );
              } else {
        ...
        // Also check for any remaining original rows with numeric indices that weren't matched
        for (const [, origRow] of originalRowMap.entries()) {
          if (!processedOriginalRows.has(origRow)) {
            const deletedRow = markRowAsDeleted(origRow);
            if (deletedRow) {
              // Append at end since we don't have a clear insertion point
              targetTbody.appendChild(deletedRow);
            }
          }
        }
      ...
      const processSimpleTable = (originalTable, editedTable) => {
        ...
        // Track indices of rows we've processed
        const processedOriginalRows = new Set();
        ...
        for (let rowIndex = 0; rowIndex < minRowCount; rowIndex++) {
          ...
          processedOriginalRows.add(rowIndex);
        }
        ...
        // Handle deleted rows (rows in original that aren't in edited)
        if (originalRows.length > editedRows.length) {
          // Find the tbody where we should insert deleted rows
          let targetTbody = editedTable.querySelector("tbody");
          if (!targetTbody) {
            // If no tbody exists, create one
          ...
          // Insert deleted rows from original table
          for (
            let rowIndex = minRowCount;
            ...
            // Insert the deleted row at the appropriate position
            if (rowIndex >= editedRows.length) {
              // If we're adding at the end, just append
              targetTbody.appendChild(deletedRow);
            } else {
              // Otherwise try to insert at an appropriate position
              targetTbody.insertBefore(
                deletedRow,
                editedRows[rowIndex] || null
              );
      ...
      const compareNonIndexedRows = (
        originalRows,
        editedRows,
        matchedEditedRows = new Set(),
        matchedOriginalRows = new Set()
      ) => {
        ...
        originalRows.forEach((origRow) => {
          ...
          if (bestMatch && highestSimilarity > 0.5) {
            ...
            if (matchedEditedRows) matchedEditedRows.add(bestMatch);
            if (matchedOriginalRows) matchedOriginalRows.add(origRow);
          }
        });
      };
      ...
      // Function to mark a row as deleted (red)
      const markRowAsDeleted = (row) => {
        if (!row) return;

        // Clone the row to insert into the edit document
        const clonedRow = row.cloneNode(true);

        // Apply deleted styling to all cells in the cloned row
        const cells = clonedRow.querySelectorAll("td, th");
        cells.forEach((cell) => {
          if (cell) {
            const content = cell.innerHTML;
            cell.innerHTML = `<span style="background-color: #FFCCCC; color: red; text-decoration: line-through; display:inline-block; width:100%">${content}</span>`;
          }
        });
